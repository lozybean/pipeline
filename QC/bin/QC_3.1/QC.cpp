...
void usage ()
{
	cout << "\nversion_3.1" 
			...
			<< "  -A <int>  filter reads with polyA, set a cutoff >=5" << endl
			<< "  -T <int>  filter reads with polyT, set a cutoff >=5" << endl
...
int main(int argc, char *argv[])
{
	...
	int c;
	while ((c=getopt(argc,argv,"a:b:c:d:q:e:N:Q:A:T:B:goxwyh")) != -1)
	{
		switch (c)
		{
			...
			case 'T' : PolyT = atoi(optarg);break;
	...
	if(PolyA>0 && PolyA<5) PolyA = 5;
	if(PolyT>0 && PolyT<5) PolyT = 5;
	...
	if(IF_GZ_OUT)
	{
		string seq1_clean = out_pre + ".1.fq.gz";
		string seq2_clean = out_pre + ".2.fq.gz";
		string single_clean = out_pre + ".single.fq.gz";
		...
		if ( ! gzcleans)
		{
			...
			exit(1);
                ...
                string seq1_clean = out_pre + ".1.fq";
                string seq2_clean = out_pre + ".2.fq";
                string single_clean = out_pre + ".single.fq";	
	...
	if(!IF_GZ)
	{
		...
		{	
				...
				p1 = filter_low_qual(seq11,qual11,clean1_Q20,N_num,Qual_rate,Q_SHIFT,Q_LOW,PolyA,PolyT,Do_trim_lowq,Do_trim_5_N,Trim_3_Low);
				p2 = filter_low_qual(seq22,qual22,clean2_Q20,N_num,Qual_rate,Q_SHIFT,Q_LOW,PolyA,PolyT,Do_trim_lowq,Do_trim_5_N,Trim_3_Low);
