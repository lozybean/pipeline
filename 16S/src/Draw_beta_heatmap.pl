...
Usage:
	...
	-g [s]	Name of group colunm, A(Area), T(Time) or A,T
...
my $R_pars = ($sample_num <= 10) ? "fontsize=7, cellwidth=12, cellheight=9" : ($sample_num>10 && $sample_num <=20) ? "cellwidth=12, cellheight=9" : "";
my $png_pars = ($sample_num <= 10) ? "width=500, height=500" : ($sample_num>10 && $sample_num <=20) ? "width=600, height=600" : "width=1000, height=1000";
...
pdf("$dir/Beta_diversity_heatmap.pdf",11,8.5)
#par(mfrow=$mfrow)
for (i in 1:length(file)) {
	infile <- read.table(file[i],header=T,check.names=F)
...
`/data_center_01/home/NEOLINE/wuleyun/wuly/R-3.1.2/bin/Rscript $dir/Beta_diversity_heatmap.r`;
