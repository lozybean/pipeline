...
statsKWs<-cbind(rownames(X),means,fdr)
ploc=ncol(statsKWs)
colnames(statsKWs)<- c("taxonname",meanname,"qvalue")
statsKWs<-statsKWs[statsKWs[,ploc]<$qcutoff,]
write.table(statsKWs,"$otuname.diff.marker.txt",row.names=F,quote=F,sep="\t")
RTXT
