...
for(j in 1:length(xlist)){
	area=as.character(g_order[j])
	mean=mean(as.matrix(xlist[[j]]),na.rm=T)
	means=cbind(means,mean)
	meanname=c(meanname,paste("mean(",area,")",sep=""))
       }

pdf("$alphaname.boxplot.pdf",11,8.5)
y1=min(min(X,na.rm=T))
y2=max(max(X,na.rm=T))
par(mar=c(4.1,5.1,4.1,1.1))
library(RColorBrewer)
#cols=colorRampPalette(brewer.pal(9, "Set1"))(9)
xlist
boxplot(xlist,ylim=c(y1,y2),ylab=expression(paste(alpha, " Diversity ($alphabase diversity index)",sep="")),col=gcols_order,xaxt="n",cex.lab=1.6,cex.axis=1.2)
axis(side=1,at=1:length(xlist),labels=g_order,cex.axis=1.2)
if($group_number==2){
