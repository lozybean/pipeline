...
p <-ggplot(data, aes(Tax,Percent,fill=Group))
p+geom_boxplot(outlier.size=1)+theme(axis.text=element_text(colour="black"),axis.text.x  = element_text(angle=60, size=8,vjust=0.5))+scale_fill_discrete(name="Group",h=c(100,1000),c=100,l=60)+labs(title="",x="",y = "Relative Abundance")+theme(legend.position=c(0.95,0.85))
dev.off()
RTXT
